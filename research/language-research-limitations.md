...
2. **Sign languages** (5 entries + ISL): no WFD/deaf-org consultation yet;
   Afghan signing treatment disputed; ISL dialect spread unresolved. Text-first
   readiness cap (PARTIAL) is a policy choice, documented in METHOD.
3. **Macro/code disputes**: mni members, hye members (both OPEN in build
   review); Rajasthani macro; Hazaragi haz (may deserve PK entry);
   Pothohari pho; Marwari/Dhatki/Parkari Koli threshold batch (PK long-tail).
...
2. SIL iso639-3.tab bulk check green (OPEN item closed).
3. UNESCO WAL endangerment flags joined; WFD sign review done.
