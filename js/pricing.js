...
(function () {
  ...
  var WANTED = stored() || COUNTRY_CUR[COUNTRY] || "USD";   // what we would like to show
  var CURRENCY = WANTED;
  ...
  /* ---------------------------------------------------------
     LIVE RATES ARRIVING LATE
     ---------------------------------------------------------
     js/rates.js fetches today's real rates. A fresh cache is
     applied before this file even runs, so usually there is
     nothing to do. But on a first visit, or when the cache has
     expired, the numbers land a moment after the page has
     already drawn its prices. When that happens we redraw.

     It also rescues a currency that had no typed rate at all:
     the live feed carries 166 currencies against the 50 in
     site-config.js, so a Ghanaian or Vietnamese visitor who
     was silently downgraded to USD now gets their own money.
     --------------------------------------------------------- */
  function onLiveRates() {
    var before = CURRENCY;

    /* the visitor's real preference may now be servable */
    if (MODE !== "off" && CURRENCY === "USD" && WANTED !== "USD" && RATES[WANTED]) {
      CURRENCY = WANTED;
    }
    ...
    if (before !== CURRENCY) {
      /* the currency switcher label is now wrong — let the UI know */
      try {
        window.dispatchEvent(new CustomEvent("ekguru:currency", { detail: CURRENCY }));
      } catch (e) {}
    }
  }

  /* Every price on the page is tagged with its USD value, so a
     redraw is just a re-read of that attribute. Elements the
     scripts rendered are tagged by main.js and features.js; the
     pre-rendered static pages carry data-usd in their HTML. */
  function redrawPrices() {
    try {
      var nodes = document.querySelectorAll("[data-usd]");
      for (var i = 0; i < nodes.length; i++) {
        var el = nodes[i];
        var u = Number(el.getAttribute("data-usd"));
        if (isNaN(u)) continue;

        /* Some prices sit inside a phrase — "Up to $12", "$8 · 50 min".
           The wording is kept in its own attribute so a redraw
           rewrites the number without eating the words around it. */
        var pre = el.getAttribute("data-usd-prefix") || "";
        var suf = el.getAttribute("data-usd-suffix") || "";
        var mode = el.getAttribute("data-usd-mode");

        if (mode === "full") {
          /* priceWithUsd returns markup, so this one has to be innerHTML */
          el.innerHTML = pre + priceWithUsd(u) + suf;
        } else if (mode === "bare") {
          /* just the converted figure, no "($8)" tail */
          el.textContent = pre + price(u) + suf;
        } else {
          el.textContent = pre + price(u) + (isConverted() ? " ($" + u + ")" : "") + suf;
        }
      }
    } catch (e) {}
  }

  window.addEventListener("ekguru:rates", onLiveRates);

  window.EkGuruPrice = {
    price: price,
    redraw: redrawPrices,
    /* Where today's numbers came from, for the footer note and
       for anyone auditing the prices. */
    rateInfo: function () {
      return window.EKGURU_RATE_INFO ||
        { live: false, source: "js/site-config.js (typed table)", count: Object.keys(RATES).length };
    },
