...
graph_report = {
    ...
    "betaStarterLanguages": sorted(l["id"] for l in (langs or {}).get("languages", []) if l["productionStatus"] == "BETA"),
    "note": "The graph holds Hindi's real content (364 entities). Language packs live beside the graph as BETA starter content and are indexed by the search + language-pack loader, not added to the graph as fake course entities.",
}
...
for l in (langs or {}).get("languages", []):
    ...
    elif l.get("starterPack"):
        entry["starterCounts"] = l.get("starterCounts")
        entry["evidence"] = {
            "langs_test": langs_test and langs_test.get("pass"),
            "note": "BETA starter pack (24 words + 12 phrases + 4 grammar) rendered through the shared engines in real Chromium (Gate J) — reference content, not a course.",
        }
    else:
        entry["evidence"] = "no content — PLANNED, never marked available"
...
items = [
    ...
    ("language registry integrated", "GREEN", "30 langs, only hi PRODUCTION + 29 BETA starter packs, 0 PLANNED, rendered in admin"),
    ...
    ("multi-language starter packs + home reachability (Stage 3)", "GREEN", "29 authored BETA packs each with a unique indexable /languages/{code}/ page + home section/nav/global-nav links; no PLANNED language gets a thin page (Gate J)"),
    ...
    ("all 30 registered languages have authored starter packs (Stage 5)", "GREEN", "29 BETA packs (incl. fr/ar/de/ja/ko/zh/ru/pt/it/nl/pl/tr/fa/he/th/vi/id/ms/sw/uk) each 24 words + 12 phrases + 4 grammar; RTL tags fixed ar-SA/fa-IR/he-IL/ms-MY; hi stays the only PRODUCTION language (Gate L)"),
    ("global nav renamed to 'Learn languages' (Stage 6)", "GREEN", "'More on EkGuru' block -> 'Learn languages' heading + aria-label on home and every inner page; first entry links /languages/ (Gate M)"),
    ("voice everywhere, honest (Stage 6)", "GREEN", "languages hub mounts a browser-TTS Listen button for every one of the 30 language cells (correct BCP-47 tags: ar-SA/fa-IR/he-IL/zh-CN…); SRS review card and vocabulary practice lab speak the Devanagari word — computer voice, never native (Gate M)"),
    ("practice questions rotate daily, deterministically (Stage 6)", "GREEN", "dayShuffle: same date + salt -> same set, different date -> different set (rule-based, not AI, not random); honest note on quiz + daily practice (Gate M)"),
    ("first full non-Hindi course delivered (Stage 7, Spanish)", "GREEN", "authored Spanish course: 6 lessons, practice lab (12 vocab + 12 grammar) on the shared engine with es-ES computer-voice Listen, 20-question daily-rotating topic quiz, 24-card SRS review deck — verified in real Chromium, 0 page errors (Gate N); es stays BETA, hi stays PRODUCTION"),
    ("second full course delivered, generator generalized (Stage 8, French)", "GREEN", "course generator refactored to one language-parameterized engine over tools/course-data/{code}.py; authored French course (6 lessons, 12 vocab + 12 grammar, 20-question daily-rotating quiz, 24-card SRS review) verified in real Chromium with fr-FR computer voice, 0 page errors (Gate N now iterates every course in data/courses.json); fr stays BETA"),
    ...
    ("SEO scale test with real numbers", "GREEN", "655 pages, 647 indexable, 0 duplicate titles/descs, 0 broken links, 0 orphans (1 known canonical drift /tutor.html -> /tutor/)"),
    ("automation/generation pipeline", "GREEN", "24/24 stages PASS (run-phase7c-pipeline.py)"),
...
handoff = {
    ...
    "nextExactCommand": "python3 tools/build-language-course.py && python3 tools/build-phase7c-registries.py && python3 tools/build-phase7-pages.py && python3 tools/build-hindi-pages.py && python3 tools/test-phase7c-stage7.py",
