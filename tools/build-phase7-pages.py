#!/usr/bin/env python3
"""Maintain the Phase-7 homepage language rail and global navigation.

The legacy language-page routines remain below for forensic reproducibility,
but their aspirational AVAILABLE/BETA registry is not publication evidence.
The default entry point therefore only updates the publication-gated homepage
rail and conservative navigation copy; it never republishes research pages.
"""
...
def patch_home():
    ...
    a nav link and a publication-gated language-course section. Research-only
    languages are deliberately omitted. Markers keep the replacement idempotent."""
    path = "index.html"
    html = open(path, encoding="utf-8").read()
    orig = html
    # The old Phase-7 registry recorded aspirational AVAILABLE/BETA labels and
    # is not publication evidence. Build this rail only from the conservative
    # course publication gate used by the course hub and player.
    try:
        courses = json.load(open("data/courses/index.json", encoding="utf-8"))["courses"]
    except Exception:
        courses = []
    published = sorted([course for course in courses if course.get("levels")], key=lambda item: item.get("name") or item["code"])
    changed = False
    ...
    # 2) Publication-gated language course rail.
    pills = ['<a class="lang-pill on" href="/courses/">Course library <small>current catalogue</small></a>']
    for course in published:
        code = course["code"]
        levels = list((course.get("levels") or {}).keys())
        label = "complete A1–C2" if course.get("complete") else "published " + " · ".join(levels)
        course_path = "languages/%s/course/index.html" % code
        href = "/languages/%s/course/" % code if os.path.exists(course_path) else "/languages/%s/level/" % code
        pills.append('<a class="lang-pill on" href="%s">%s <small>%s</small></a>' %
                     (esc(href), esc(course.get("name") or code), esc(label)))
    pills = "".join(pills)
    section = (
        ...
        '      <span class="xp-kicker">Published language courses</span>\n'
        '      <h2>Only levels that pass the publication gate</h2>\n'
        '      <p>Complete and partial labels show the exact range available now. Research-only languages stay out of this rail.</p>\n'
    ...
    if 'href="languages/"' not in html.split('ekguru:global-nav:start')[1].split('ekguru:global-nav:end')[0]:
        html = html.replace(
            '<ul class="eg-gn-list">',
            '<ul class="eg-gn-list">\n'
            '    <li><a href="languages/"><b>Learn languages</b><span>Publication-gated courses with exact level ranges. Research-only languages are excluded.</span></a></li>', 1)
        changed = True

    conservative_copy = {
        "Every EkGuru student gets a dedicated Hindi teacher. Speak, read and write with real confidence — through live 1-on-1 lessons built around your goals.":
            "Browse current Hindi tutor profiles, compare their published details and contact the tutor whose approach fits your goal.",
        "Four things every learner gets, from the first message to the hundredth lesson.":
            "What to check before contacting a tutor.",
        '<h3 data-i18n="why.w1t">Personal plan</h3>':
            '<h3 data-i18n="why.w1t">Published profiles</h3>',
        '<h3 data-i18n="why.w2t">Speaking first</h3>':
            '<h3 data-i18n="why.w2t">Lesson approach</h3>',
        '<h3 data-i18n="why.w3t">Genuinely affordable</h3>':
            '<h3 data-i18n="why.w3t">Current terms</h3>',
        '<h3 data-i18n="why.w4t">Flexible timing</h3>':
            '<h3 data-i18n="why.w4t">Confirm availability</h3>',
        "Every learner has a different goal — every lesson is built around yours.":
            "Use the published profile, subjects and level information to find a suitable tutor.",
        "You start speaking Hindi in your very first class. Hesitation disappears fast.":
            "Ask the tutor how speaking, reading and writing will be balanced for your goal.",
        "Lessons from $6. Low price, zero compromise on teaching quality.":
            "Check the current price, duration and booking terms on the individual tutor profile.",
        "IST, US, Europe, Gulf or Japan — pick a slot that works in your timezone.":
            "Confirm a current lesson time directly with the tutor before booking.",
        'EkGuru is built and run by <b>Prakash</b> (B.Tech, MNIT Jaipur) — one person,\n      no middleman, no platform fee. <a href="about/">Read the full story →</a>':
            '<a href="about/">Read EkGuru’s ownership, editorial and funding disclosures →</a>',
        '<span class="xp-kicker">Where students learn</span>':
            '<span class="xp-kicker">Country-language research</span>',
        '<h2 data-i18n="markets.title">Students from around the world</h2>':
            '<h2 data-i18n="markets.title">Documented country-language relationships</h2>',
        'See <a href="learn-hindi-by-country/">every country we teach</a>.':
            'See the <a href="learn-hindi-by-country/">country-language research registry</a>.',
        '<a class="btn btn-ghost" href="learn-hindi-by-country/">Every country we teach →</a>':
            '<a class="btn btn-ghost" href="learn-hindi-by-country/">Review the research registry →</a>',
        "Each tutor sets their own price and every profile shows it in your own currency — including the cheapest lesson currently available. No platform fee is added on top.":
            "Check the current price, duration, currency and booking terms on the individual tutor profile before contacting the tutor.",
        "No. Our tutors teach complete beginners regularly, starting from the Devanagari alphabet and everyday greetings.":
            "Tutor profiles state the levels they support. Check the profile and confirm that the tutor currently accepts complete beginners.",
        "Yes. A trial lesson is available so you can decide after your very first class, with no obligation.":
            "Check the individual tutor profile for current trial terms before sending a request.",
        "Lessons are live and one-to-one over video call. You agree the platform and timing directly with your tutor.":
            "Ask the tutor which lesson platform and format they currently offer, then agree the timing directly.",
        "Our tutors teach students in the United States, Spain, France, Germany, Brazil, Japan and the UAE, with slots arranged around your local time.":
            "Availability depends on the individual tutor. Confirm the timezone and current lesson time directly before booking.",
        "Yes. Listing is free with no commission — send your bio, photo, intro video and rates from the Become a Tutor page.":
            "Use the Become a Tutor page to submit profile information. Submission does not guarantee publication."
    }
    for old, new in conservative_copy.items():
...
def reconcile_global_nav():
    """Remove launch claims from every existing generated global-nav block."""
    copy = {
        "Learn languages": "Publication-gated courses with exact level ranges. Research-only languages are excluded.",
        "Hindi from your first language": "Quarantined source-language research. Incomplete relationships remain unpublished.",
        "Learn Hindi from your country": "Quarantined country-language research with verified and provisional records clearly labelled.",
        "Talk to a person": "Questions about lessons, accessibility, privacy or corrections.",
        "Who runs EkGuru": "Ownership, editorial approach, funding disclosures and contact information."
    }
    changed = 0
    for root, dirs, files in os.walk("."):
        dirs[:] = [d for d in dirs if d not in {".git", "node_modules", ".venv"}]
        for name in files:
            if name != "index.html" and not name.endswith(".html"):
                continue
            path = os.path.join(root, name)
            text = open(path, encoding="utf-8").read()
            if "ekguru:global-nav:start" not in text or "ekguru:global-nav:end" not in text:
                continue
            start = text.index("<!-- ekguru:global-nav:start")
            end = text.index("<!-- ekguru:global-nav:end -->", start)
            block = text[start:end]
            updated = block
            for label, description in copy.items():
                pattern = r"(<b>" + re.escape(label) + r"</b><span>).*?(</span>)"
                updated = re.sub(pattern, lambda match, value=description: match.group(1) + value + match.group(2), updated)
            if updated != block:
                text = text[:start] + updated + text[end:]
                with open(path, "w", encoding="utf-8") as handle:
                    handle.write(text)
                changed += 1
    print("reconciled: %d generated global-nav blocks" % changed)


def main():
    # Fail closed: the old registry-driven page builders can expose research
    # material, so they are intentionally not part of the default command.
    patch_home()
    reconcile_global_nav()
    print("publication-gated home and navigation synced; research pages unchanged")
