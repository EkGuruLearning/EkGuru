...
(function () {
  ...
  /* Records what visitors do. Wrapped so a logging failure can never
     interrupt a booking.

     BUG FOUND v48 — this function wrote to EkGuruStore and NOWHERE
     ELSE. EkGuruStore is localStorage, i.e. one browser: yours. So
     the site tracked thirteen meaningful events (booking_sent,
     booking_abandoned, search, video_play, share…) and every single
     one died inside the visitor's own machine. The day GoatCounter
     was switched on it would have shown page views and not one
     conversion, and the reason would have been invisible.

     Both destinations now, always:
       EkGuruStore     the local ledger the admin Activity tab reads
       EkGuruAnalytics GoatCounter, so the numbers are site-wide

     The analytics half no-ops by itself when analytics is off — the
     object simply does not exist — so this is safe either way. */
  function track(event, detail) {
    try { if (window.EkGuruStore && window.EkGuruStore.track) window.EkGuruStore.track(event, detail); }
    catch (e) {}
    try {
      if (window.EkGuruAnalytics && window.EkGuruAnalytics.event) {
        /* GoatCounter groups by the path string, so the label must be
           short and low-cardinality or the dashboard turns into a list
           of thousands of one-off rows. Tutor id yes, search text no. */
        var label = "";
        if (detail) {
          if (detail.tutor) label = String(detail.tutor);
          else if (detail.where) label = String(detail.where);
          else if (detail.reason) label = String(detail.reason).slice(0, 40);
        }
        window.EkGuruAnalytics.event(event, label);
      }
    } catch (e) {}
  }
  ...
  function attachSearch(input, opts) {
    ...
    function render(q) {
      ...
      /* Log the query once the visitor stops typing, not per keystroke. */
      clearTimeout(searchTimer);
      searchTimer = setTimeout(function () {
        if (q.trim().length >= 2) track("search", { q: q.trim().slice(0, 40), hits: results.length });
      }, 900);

      if (!results.length) {
        /* A search that finds nothing is the single most actionable
           number on the dashboard: it is a person telling you what
           they wanted and you not having it. */
        track("search_empty", { q: q.trim().slice(0, 40) });
      ...
      box.innerHTML =
        ...
        results.slice(0, 5).map(function (x, i) {
          return '<a class="ac-item" role="option" data-i="' + i + '" href="' +
            ...
            '<span class="ac-price" data-usd="' + esc(x.priceUSD) + '" data-usd-mode="bare">' + px(x.priceUSD) + "</span></a>";
        }).join("") +
  ...
  function slotLabel() {
    ...
    /* A dated slot carries a real Date, so the email can name the
       actual day. "Mon 16:00" needed the tutor to work out which
       Monday; "Wed 10 Sep, 16:00" does not. */
    if (chosen.dated && chosen.date) {
      var d = chosen.date;
      var MONS = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];
      var p2 = function (x) { return x < 10 ? "0" + x : String(x); };
      /* BUG FOUND v53 — OFF-BY-ONE ON THE WEEKDAY NAME.

         features.js indexes DAYS from MONDAY:
             DAYS = ["Mon","Tue","Wed","Thu","Fri","Sat","Sun"]
         because that is the order the availability grid is drawn
         in. But Date#getDay() counts from SUNDAY (0 = Sunday).

         Passing getDay() straight into DAYS shifted every name by
         one, so Wednesday 9 September was emailed to the tutor as
         "Thu 9 Sep". The date was right and the day name was
         wrong — the single most confusing way to get a booking
         wrong, because each half looks correct on its own.

         DAY_FROM_SUNDAY maps getDay() onto the same names without
  ...
  /* Opens the student's own email app with everything pre-written.
     Used when background sending is off, or as the rescue route if
     the mail relay refuses — a request must never just vanish. */
  function mailtoHref() {
    var to = modalTutor.email || SITE.email;
    var subj = "Lesson booking request — " + modalTutor.name + " (EkGuru)";
    var cc = [];
    if (SITE.email && String(SITE.email).toLowerCase() !== String(to).toLowerCase()) cc.push(SITE.email);
    return "mailto:" + to +
  ...
  /* Can we deliver the mail ourselves, in the background? */
  function canSendDirect() {
    return !!(window.EkGuruMail && window.EkGuruMail.available());
  }

  function buildActions() {
    var host = $(".bk-actions", modal);
    if (!host) return;
    if (host.dataset.sending === "1") return;   // never redraw mid-send
    var direct = canSendDirect();
    var mail = mailtoHref();
    var wa = hasWa(modalTutor.whatsapp) ? modalTutor.whatsapp : (hasWa(SITE.whatsapp) ? SITE.whatsapp : "");
    host.innerHTML =
      '<button class="btn btn-primary btn-lg bk-send" type="button" data-href="' + esc(mail) + '">✉️ ' +
        esc(t(direct ? "book.sendRequest" : "book.sendEmail")) + "</button>" +
      (wa ? '<a class="btn btn-wa btn-lg" data-track-wa="' + esc(modalTutor.id) + '" target="_blank" rel="noopener" href="https://wa.me/' + String(wa).replace(/\D/g, "") +
    ...
    $(".bk-send", host).addEventListener("click", function () {
      if (!validate()) return;
      if (!canSendDirect()) { location.href = this.dataset.href; return; }
      sendRequest(this);
    });
  ...
  /* =========================================================
     4b. SENDING THE REQUEST FOR REAL
     ---------------------------------------------------------
     One press delivers the same message to the tutor's own
     inbox, to EkGuru, and back to the student as their receipt.
     The delivery itself lives in js/mailer.js. If anything at
     all goes wrong the student is told why and handed their own
     email app instead, with the message still written out.
     ========================================================= */
  function sendRequest(btn) {
    var host = $(".bk-actions", modal);
    var err = $(".bk-err", modal);
    var fallbackHref = btn.dataset.href;

    host.dataset.sending = "1";
    btn.disabled = true;
    ...
    /* The reference is minted before the send, not after, so the
       same code appears in the email, on the student's screen and
       in the ledger. Without this the email had no reference at
       all and the three could not be matched up. */
    var ref = "";
    try {
    ...
    window.EkGuruMail.send({
      ...
      slot: chosen ? slotLabel() : "",
      price: priceLine(modalTutor.priceUSD),
      message: buildMessage(),
      pageUrl: location.href
    }).then(function (res) {
      host.dataset.sending = "";
      /* Written to the browser's own booking ledger too, so the
  ...
  function logBooking(status, res, err, ref) {
    try {
      ...
      return window.EkGuruStore.add({
        ...
        goal: (($("#bk-goal") || {}).value || "").trim(),
        slot: chosen ? slotLabel() : "",
        priceUSD: modalTutor.priceUSD,
        priceShown: priceLine(modalTutor.priceUSD),
        page: location.href,
        status: status,
        error: err || "",
  ...
  /* ═══════════════════════════════════════════════════════
     v93 — THE SAME BOOKING, IN THE UNIFIED LEDGER
     ───────────────────────────────────────────────────────
     js/store.js above is the BOOKING diary and stays exactly as
     it is — the dashboard's Bookings tab, the CSV export and the
     revenue figures all read it.

     js/ledger.js is a different thing: one index of every
     message the site sends, of any kind, so a reference can be
     looked up whether it came from a booking, the contact form,
     an admin reply or a bulk send. Before v93 a contact
     reference existed on the visitor's screen and nowhere else.

     Both are written. Duplicating four fields is cheaper than
     merging two records with different shapes and different
     readers, and store.js has six months of assumptions built on
     it that are not worth disturbing for tidiness.

     Records WHO GOT WHAT, which is the question Prakash keeps
     asking and which neither store nor the inbox answers on its
     own: three recipients, named, with the relay that carried
     each one. */
  function logLedger(ok, res, err, ref) {
    try {
      if (!window.EkGuruLedger) return null;
      var who = [];
      if (res) {
        /* mailer.send() resolves with the tutor address in `to`
      ...
      return window.EkGuruLedger.add({
        ...
        tutor: modalTutor.name,
        tutorId: modalTutor.id,
        subject: "Booking request — " + modalTutor.name,
        summary: (chosen ? slotLabel() + " · " : "") +
  ...
  /* "$8", or "$8 (about ₹664)" when the visitor is seeing their
     own currency. Written defensively so a missing pricing
     module can never block a booking. */
  function priceLine(usd) {
    try {
      if (window.EkGuruPrice && window.EkGuruPrice.isConverted()) {
        return "$" + usd + " (about " + px(usd) + ")";
      }
    } catch (e) {}
    return "$" + usd;
  }

  /* Replaces the form with a plain confirmation that names every
     inbox the request reached, so the student can see their own
     copy is on the way and knows who has it. */
  /* Replaces the form with a receipt that answers the three
     questions plainly: who sent it, who received it, and what was
     asked for. The same three lines head the email, so a student
     and a tutor comparing notes see identical wording. */
  function showSent(res, rec) {
    var body = $(".bk-body", modal);
    if (!body) return;

    var name = (($("#bk-name") || {}).value || "").trim();
    var mail = (($("#bk-email") || {}).value || "").trim();
    var ref = (rec && rec.ref) || (res && res.ref) || "";

    var from = name + (mail ? " (" + mail + ")" : "");
    ...
    /* Every inbox that received a copy, named rather than listed
       as a bare address, so it is obvious who has seen it. */
    var mineLc = mail.toLowerCase();
    var siteLc = String(SITE.email || "").toLowerCase();
    /* Named, not listed as raw addresses. The student sees their own
       address (they typed it) and the platform name, never another
       person's inbox. */
    var copies = ((res && res.cc) || []).map(function (a) {
      var k = String(a).toLowerCase();
      if (k === mineLc) return t("book.sentYou") + " — " + a;
      if (k === siteLc) return SITE.brand || "EkGuru";
      return SITE.brand || "EkGuru";
    });
    ...
    /* ═══════════════════════════════════════════════════════
       ...
       This list is built from res.cc. When a tutor's booking
       goes to EkGuru's own address — which is now the default,
       because publishing a personal inbox on a public page is
       not acceptable — mailer.js correctly de-duplicates the
       platform copy out of cc, since it is already the main
       recipient. The student was then told their request went
       to the tutor and to themselves, and nothing about a
       record being kept.

       That is a real gap, not a cosmetic one: a student who
       cannot see that the platform has a copy has no reason to
       believe anyone will follow up if the tutor does not.

       So if the platform is the primary recipient, say so
       explicitly rather than inferring it from a list that
       correctly does not contain it.
       ═══════════════════════════════════════════════════════ */
    var brandName = SITE.brand || "EkGuru";
    var primaryLc = String((res && res.to) || "").toLowerCase();
    if (primaryLc && primaryLc === siteLc && copies.indexOf(brandName) === -1) {
      copies.unshift(brandName);
    }

    function row(label, value) {
      return '<div class="bk-rc-row"><span>' + esc(label) + "</span><b>" + esc(value) + "</b></div>";
    }

    body.innerHTML =
      '<div class="bk-done" role="status">' +
        '<div class="bk-done-ic" aria-hidden="true">✓</div>' +
        "<h4>" + esc(t("book.sentTitle")) + "</h4>" +
        "<p>" + esc(t("book.sentBody").replace("{tutor}", modalTutor.name)) + "</p>" +

        '<div class="bk-receipt">' +
          (ref ? row(t("book.rcRef"), ref) : "") +
          row(t("book.rcFrom"), from || "—") +
          row(t("book.rcTo"), to) +
          row(t("book.rcWhat"), what) +
          row(t("book.rcWhen"), new Date().toLocaleString()) +
        "</div>" +

        (copies.length
          ? '<p class="bk-rc-cap">' + esc(t("book.rcCopies")) + "</p>" +
        ...
        '<div class="bk-done-btns">' +
          '<button class="btn btn-ghost bk-rc-copy" type="button">📋 ' + esc(t("book.rcCopy")) + "</button>" +
          '<button class="btn btn-primary bk-done-x" type="button">' + esc(t("book.close")) + "</button>" +
        "</div>" +
      "</div>";

    /* A copy button, so the student can keep the receipt without
       waiting for the email to arrive. */
    var cp = $(".bk-rc-copy", body);
    if (cp) cp.addEventListener("click", function () {
      var txt = t("book.rcRef") + " " + ref + "\n" +
        t("book.rcFrom") + " " + from + "\n" +
        t("book.rcTo") + " " + to + "\n" +
        t("book.rcWhat") + " " + what + "\n" +
        t("book.rcWhen") + " " + new Date().toLocaleString() +
        (copies.length ? "\n" + t("book.rcCopies") + " " + copies.join(", ") : "");
      function done() {
        cp.textContent = "✓ " + t("book.copied");
        setTimeout(function () { cp.textContent = "📋 " + t("book.rcCopy"); }, 1800);
      }
      if (navigator.clipboard && navigator.clipboard.writeText) {
    ...
    var x = $(".bk-done-x", body);
    if (x) { x.addEventListener("click", closeBooking); x.focus(); }
  }
  ...
  function renderSlots() {
    ...
    /* =========================================================
       REAL DATED SLOTS  (v53)
       ---------------------------------------------------------
       The original picker showed a repeating weekly grid — "Mon
       16:00", "Tue 09:00". That is what the tutor file contains,
       so it was the honest minimum, but it made the student do
       the work: which Monday? is that my time or theirs? has
       that already passed today?

       js/schedule.js turns the same data into real dates for the
       next fourteen days, already converted into the visitor's
       own timezone, with anything less than three hours away
       removed. Same source data, far less thinking required.

       The old weekly grid is kept below as a fallback. If
       schedule.js is missing — an old cached page, a blocked
       file — the modal still works exactly as it did before
       rather than showing nothing. A booking form that renders
    ...
    if (window.EkGuruSchedule) {
      var n = window.EkGuruSchedule.render(host, modalTutor, function (dt, label) {
        ...
        /* BUG FOUND v53 — THE ACTIVITY LOG LOST slot_picked.

           The old weekly grid called track(), which writes to BOTH
           the local ledger (the admin Activity tab) and analytics.
           schedule.js only fires an analytics event, because it has
           no business knowing about the ledger.

           So switching pickers silently dropped slot_picked from the
           admin dashboard — the funnel would have shown people
           opening the booking box and sending requests with nothing
           in between. Logging it here, where the ledger is in scope,
    ...
    $all(".bk-slot", host).forEach(function (b) {
      b.addEventListener("click", function () {
        ...
        track("slot_picked", { tutor: modalTutor.id, slot: b.dataset.time });
        $(".bk-err", modal).hidden = true;
        updatePreview();
      });
    });
  ...
  function openBooking(id) {
    ...
    track("booking_open", { tutor: modalTutor.id });
    /* If the last visit ended on the "request sent" screen, throw
       that modal away so a fresh, empty form is built. */
    if (modal && $(".bk-done", modal)) {
    ...
    modal.hidden = false;
    /* =========================================================
       BUG FOUND v82 — THE BOOKING MODAL HAD THE v74 BUG TOO
       ---------------------------------------------------------
       This used `body.no-scroll { overflow:hidden }`, which is
       exactly the technique v74 removed from the hamburger for
       being broken on iOS Safari:

         · overflow:hidden on body stops it being the scroll
           container, so scrollY collapses to 0
         · html { scroll-behavior:smooth } then ANIMATES that
           collapse, so the page visibly slides to the top behind
           the modal
         · closing it leaves the reader wherever the collapse
           dumped them, not where they were

       v74 fixed the drawer and left the modal, because the
       drawer was what had been reported. Same bug, same page,
       one function away — and it is worse here, because a
       booking modal is opened from a tutor card that is usually
       well down the page.

       Uses the same mechanism as the drawer now: position:fixed
       plus a compensating offset, restored on close. Written
       once in lockScroll()/unlockScroll() below so the two
  ...
  /* ---------------------------------------------------------
     ONE scroll lock, shared.
     ---------------------------------------------------------
     ⚠️ Do NOT reintroduce overflow:hidden. See the note above
     and tools/test-hamburger.js, which asserts against it.

     Guarded with a counter because the drawer and the modal can
     both be open in principle; unlocking on the first close
     would release the page while the second is still up.
     --------------------------------------------------------- */
  ...
  function unlockScroll() {
    ...
    /* behavior:"auto" defeats html{scroll-behavior:smooth} —
       without it the restore is animated and looks like the very
       jump this exists to prevent. */
    try { window.scrollTo({ top: lockedY, behavior: "auto" }); }
    catch (e) { window.scrollTo(0, lockedY); }
  }

  function closeBooking() {
    if (!modal) return;
    /* Closed without sending — the most useful signal there is,
       because it says the interest was real and something stopped
       them. */
    if (modalTutor && !$(".bk-done", modal)) {
      track("booking_abandoned", { tutor: modalTutor.id, hadSlot: !!chosen });
    }
  ...
  function initBooking() {
    /* Outbound clicks worth knowing about. */
    document.addEventListener("click", function (e) {
      var wa = e.target.closest("[data-track-wa]");
      if (wa) track("whatsapp_click", { tutor: wa.getAttribute("data-track-wa") });
      var yt = e.target.closest(".pf-video, [data-yt]");
      if (yt) track("video_play", {});
      var sh = e.target.closest(".share-b");
      if (sh) track("share", { where: (sh.className.match(/share-b (\w+)/) || [])[1] || "" });
    }, true);
  ...
  function initStickyBook() {
    ...
    bar.innerHTML =
      ...
      '<div class="sb-info"><b class="sb-name">' + esc(x.name) + "</b>" +
        '<span class="sb-price" data-usd="' + esc(x.priceUSD) + '" data-usd-mode="bare" data-usd-suffix=" · ' + esc(x.lessonLength || "50 min") + '">' + px(x.priceUSD) + " · " + esc(x.lessonLength || "50 min") + "</span></div>" +
  ...
  /* =========================================================
     RE-BIND AFTER A LIVE SHEET REPAINT  (v62)
     ---------------------------------------------------------
     js/main.js now redraws the cards and the profile when the
     Google Sheet arrives, so a price or availability change shows
     without a reload. That throws away the DOM this file has
     decorated, and the first attempt at it broke three things at
     once:

       · every "Book a lesson" button inside a repainted card went
         dead — the listeners were on the elements just removed
       · the timezone bar, the review dots and the sticky booking
         bar were APPENDED again, so after two refreshes there
         were three of each stacked down the page
       · the collapsible panels on mobile lost their carets and
         every panel sat open

     Booking and live search survive on their own because they use
     delegated listeners on `document`, which no repaint can
     detach. Everything that appends an element or decorates one
     has to be torn down and rebuilt, in that order.

     Guarding this properly matters more than it looks: a
     duplicated sticky bar covers the real one and the visitor
     taps a button wired to stale data.
     ========================================================= */
  function teardownInjected() {
    /* Elements this file appends. Removing them before re-running
       the init is what makes the init safe to call twice. */
    $all(".tz-bar, .rev-dots, .sticky-book").forEach(function (el) {
      if (el.parentNode) el.parentNode.removeChild(el);
    });
    document.body.classList.remove("has-sticky-book");
    /* Panel decoration is re-applied from scratch. */
    $all("#profile .panel").forEach(function (p) {
      p.classList.remove("is-collapsible", "is-open");
      var c = $(".panel-caret", p);
      if (c && c.parentNode) c.parentNode.removeChild(c);
      var h = $("h2", p);
      if (h) { h.removeAttribute("role"); h.removeAttribute("tabindex"); h.removeAttribute("aria-expanded"); }
    });
  }

  function initSheetLive() {
    window.addEventListener("ekguru:repaint", function () {
      try { teardownInjected(); } catch (e) { warn("repaint teardown", e); }
      try { initTimezoneToggle(); } catch (e) { warn("repaint timezone", e); }
      try { initShare(); } catch (e) { warn("repaint share", e); }
      try { initReviewSlider(); } catch (e) { warn("repaint review slider", e); }
      try { initStickyBook(); } catch (e) { warn("repaint sticky book", e); }
      try { initCollapsePanels(); } catch (e) { warn("repaint panels", e); }
      try { initChips(); } catch (e) { warn("repaint chips", e); }
    });
  }
  ...
  function boot() {
    ...
    try { initSheetLive(); } catch (e) { warn("sheet live repaint", e); }
  }
